...
@section('title', 'Admin Dashboard - SATHYAS CATERING')

@section('content')
<!-- Admin Header -->
<div class="bg-[#1c120e] text-white py-10">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <p class="text-xs text-[#d4af37] font-bold uppercase tracking-widest">Admin Panel</p>
            <h1 class="text-3xl font-serif font-bold">Enquiries Dashboard</h1>
        </div>
        <div class="flex items-center space-x-4">
            <a href="{{ route('home') }}" class="text-xs text-white/80 hover:text-white border border-white/30 px-3 py-2 rounded transition">View Site</a>
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="text-xs text-white/80 hover:text-white border border-white/30 px-3 py-2 rounded transition">Logout</button>
            </form>
        </div>
    </div>
</div>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
        <div class="bg-white rounded-lg border border-[#e2d4bf] p-6 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-serif font-bold text-[#1c120e] mt-1">{{ $totalCount }}</p>
                </div>
                <div class="w-12 h-12 rounded-full bg-[#f7f2e8] flex items-center justify-center">
                    <svg class="w-6 h-6 text-[#1c120e]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-lg border border-[#e2d4bf] p-6 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-serif font-bold text-[#b8860b] mt-1">{{ $pendingCount }}</p>
                </div>
                <div class="w-12 h-12 rounded-full bg-[#f7f2e8] flex items-center justify-center">
        ...
        <div class="bg-white rounded-lg border border-[#e2d4bf] p-6 shadow-sm">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-gray-500 uppercase tracking-wider font-bold">Quotations Sent</p>
    ...
    <div class="bg-white rounded-lg border border-[#e2d4bf] shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-[#e2d4bf] bg-[#f7f2e8] flex items-center justify-between">
            <h3 class="font-serif font-bold text-lg text-[#2c221e]">All Enquiries</h3>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-[#fdfbf7] border-b border-[#e2d4bf]">
                        <tr>
                            <th class="text-left px-4 py-3 text-xs font-bold text-[#721c1c] uppercase">ID</th>
                            <th class="text-left px-4 py-3 text-xs font-bold text-[#721c1c] uppercase">Name</th>
                            <th class="text-left px-4 py-3 text-xs font-bold text-[#721c1c] uppercase">Email</th>
                            <th class="text-left px-4 py-3 text-xs font-bold text-[#721c1c] uppercase">Event</th>
                            <th class="text-left px-4 py-3 text-xs font-bold text-[#721c1c] uppercase">Date</th>
                            <th class="text-left px-4 py-3 text-xs font-bold text-[#721c1c] uppercase">Guests</th>
                            <th class="text-left px-4 py-3 text-xs font-bold text-[#721c1c] uppercase">Est. Price</th>
                            <th class="text-left px-4 py-3 text-xs font-bold text-[#721c1c] uppercase">Status</th>
                            <th class="text-left px-4 py-3 text-xs font-bold text-[#721c1c] uppercase">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-[#eee2d0]">
                        @foreach($enquiries as $enquiry)
                            <tr class="hover:bg-[#f7f2e8] transition">
                                <td class="px-4 py-3 font-semibold text-[#721c1c] text-xs">#SRI-{{ $enquiry->id }}</td>
                                <td class="px-4 py-3 font-medium">{{ $enquiry->user_name }}</td>
                                <td class="px-4 py-3 text-xs text-gray-600">{{ $enquiry->user_email }}</td>
                                <td class="px-4 py-3 text-xs">{{ $enquiry->event_type }}</td>
                                <td class="px-4 py-3 text-xs">{{ $enquiry->event_date->format('d M Y') }}</td>
                                <td class="px-4 py-3 text-xs">{{ number_format($enquiry->guest_count) }}</td>
                                <td class="px-4 py-3 text-xs font-semibold">₹{{ number_format($enquiry->estimated_price) }}</td>
                                <td class="px-4 py-3">
                                    @php
                                        $colors = [
                                    ...
                                    @endphp
                                    <span class="inline-block px-2 py-0.5 rounded-full text-[10px] font-bold border {{ $colors[$enquiry->status] ?? 'bg-gray-100 text-gray-600' }}">
                                        {{ $enquiry->status }}
                                    </span>
                                </td>
                                <td class="px-4 py-3 flex items-center space-x-2">
                                    ...
                                    <form method="POST" action="{{ route('admin.enquiry.delete', $enquiry) }}" onsubmit="return confirm('Delete enquiry #SRI-{{ $enquiry->id }}? This cannot be undone.')">
                                        @csrf
                                        @method('DELETE')
        ...
        @else
            <div class="text-center py-16">
                ...
                <p class="text-gray-400 text-sm">Enquiries from the website will appear here.</p>
            </div>
        @endif
    </div>
</div>
